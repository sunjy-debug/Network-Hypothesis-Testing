...
sgsbm_sim_algorithm = function(file_directory, output_directory, data_generation_method, random_seed, tau = c(0.005, 0.025, 0.05, 0.1, 0.15, 0.25)){
  ...
  cat("SGSBM(simultaneous) fitting begins. \n")
  start_sgsbmsim = Sys.time()
  niterations = 300
  delta = 3
  xi = 2
  rou = 0
  ...
  lambda = 1
  nrepetition = 5
  fit = SGSBM_sim_estimation(X, niterations, delta, xi, rou, kappa, alpha, beta, gamma, lambda, nrepetition)
  History = fit$Iterates
  rand_index_SGSBM_sim = rep(NA, niterations)
  if(!is.null(Z)) rand_index_SGSBM_sim = sapply(History, function(x) adj.rand.index(x$Z, Z))
  TDR_SGSBM_sim =  lapply(tau, function(x) {
  ...
  FDR_SGSBM_sim = lapply(tau, function(x) {
    infer = lapply(History, function(y) SBM_inference(y$q, x))
    FDR = sapply(infer, function(y) sum((1 - A) * y, na.rm = TRUE) / pmax(sum(y, na.rm = TRUE), 1))
    return(FDR)
  })
  end_sgsbmsim = Sys.time()
  elapse_sgsbmsim = end_sgsbmsim - start_sgsbmsim
  cat("SGSBM(simultaneous) fitting ends: \n")
  print(elapse_sgsbmsim)
  
  return(list(RI_SGSBM_sim = rand_index_SGSBM_sim, TDR_SGSBM_sim = TDR_SGSBM_sim, FDR_SGSBM_sim = FDR_SGSBM_sim, TIME_SGSBM_sim = elapse_sgsbmsim))
}

sgsbm_sim_main = function(){
  library(rslurm)
  indir <- Sys.getenv("INDIR", unset = file.path(getwd(), "Data"))
  outdir = Sys.getenv("OUTDIR", unset = file.path(getwd(), "Results/SGSBM_SIM"))
  ...
  sjob = slurm_apply(
    ...
    cpus_per_node = 1,
    global_objects = c(
      "loglike", "logmargs", "getDahl", "SGSBM_sim_estimation", "SBM_inference"
    ),
    pkgs = c(
