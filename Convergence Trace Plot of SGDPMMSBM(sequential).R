...
SGDPMMSBM = function(X, niterations, delta, xi, rou, kappa, alpha, beta, eta, zeta, m, threshold, A_true)
{
  ...
  for (niter in 1:niterations)
  {
    ...
    # true discovery rate
    TDR[niter] = sum(A_true * phi, na.rm = TRUE) / sum(as.matrix(A_true), na.rm = TRUE)
    # false discovery rate
    FDR[niter] = sum((1 - A_true) * phi, na.rm = TRUE) / pmax(sum(phi, na.rm = TRUE), 1)
    
    History[[niter]] = list(Zout = Z, Aout = A)
    if (niter %% 10 == 0) {
      end = Sys.time()
      elapse = end - start
      cat("Iteration:", niter, "ends:", elapse, "\n")
      start = Sys.time()
    }
...
ggplot(df_results, aes(x = iteration, y = TDR, color = as.factor(threshold))) +
  ...
  labs(title = "True Discovery Rate (TDR) across Iterations of SGDPMM-SBM (preferattach)", x = "Iterations", y = "TDR") +
  theme_minimal() +
  theme(panel.grid.major = element_blank())
ggsave("True Discovery Rate (TDR) across Iterations of SGDPMM-SBM (preferattach).png", width = 6, height = 4)

ggplot(df_results, aes(x = iteration, y = FDR, color = as.factor(threshold))) +
  geom_line(size = 1) +
  scale_color_viridis_d(name = "Threshold") +
  labs(title = "False Discovery Rate (FDR) across Iterations of SGDPMM-SBM (preferattach)", x = "Iterations", y = "FDR") +
  theme_minimal() +
  theme(panel.grid.major = element_blank())
ggsave("False Discovery Rate (FDR) across Iterations of SGDPMM-SBM (preferattach).png", width = 6, height = 4)
