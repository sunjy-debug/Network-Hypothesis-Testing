...
library(rslurm)
outdir <- Sys.getenv("OUTDIR", unset = file.path(getwd(), "Output/run_sgdpmmsbm_seq"))
...
ggsave(file.path(outdir, "Sequential SGDPMM-SBM Evaluation.png"), plot = p1, width = 4, height = 6, dpi = 300)
...
ggsave(file.path(outdir, "SGDPMMSBM (Sequential) ROC Curve (NSBM).png"), plot = p2, width = 4, height = 6, dpi = 300)
...
ggsave(file.path(outdir, "runtime boxplot of SGDPMMSBM (sequential).png"), p3, width = 6, height = 4, dpi = 300)
...
p4 = 
  ...
  labs(title = "Rand Index of SGDPMMSBM (sequential)", x = NULL, y = "Rand Index") +
  theme_minimal(base_size = 12) +
  theme(legend.position = "none")
ggsave(file.path(outdir, "Rand Index of SGDPMMSBM (sequential).png"), p4, width = 6, height = 4, dpi = 300)
