...
loglike = function(Z, Q, mu, sigma, X, A, i)
{
  ...
  ## output: log-likelihood related to ith observation ##
  ...
  n = nrow(X)
  c_i = Z[i] # the cluster assignment of ith observation
...
logmargs = function(Z, X, A, i, alpha, beta, rou, kappa, delta, xi)
{
  ...
  ## Output: m(X_i) in collapsed sampler for MFM-SBM ##
...
MFMSBM_seq_estimation = function(X, niterations, delta, xi, rou, kappa, alpha, beta, gamma, lambda)
{
  ...
  for (i in 1:k){
    for (j in i:k){
      ...
      Q[j,i] = Q[i,j]
      sigma[i,j] = sqrt(rinvgamma(1, delta, rate = xi))
      sigma[j,i] = sigma[i,j]
  ...
  for (niter in 1:niterations)
  {
    ...
    { #determine whether ith component is a singleton (the only one node in the cluster)
      ...
      if (counts_Z[current.cluster.i] > 1){
        ...
        if (cluster.i > k)
        {
          ...
          sigma_star[k + 1, 1:(k + 1)] = sqrt(rinvgamma(k + 1, delta, rate = xi))
    ...
    for (r in 1:k){
      ...
      for (s in r:k)
      {
        ...
        rou_t = (kappa * rou + sumX) / (kappa + sumA)
        kappa_t = kappa + sumA
        ...
        sigma[s,r] = sigma[r,s]
        mu[r,s] = rnorm(1, rou_t, sigma[r,s] / sqrt(kappa_t))
        mu[s,r] = mu[r,s]
      }
    }
...
mfmsbm_seq_algorithm = function(file_directory, output_directory, data_generation_method, random_seed, tau = c(0.005, 0.025, 0.05, 0.1, 0.15, 0.25)){
  ...
  gamma = 1
  lambda = 1
  fit = MFMSBM_seq_estimation(X, niterations, delta, xi, rou, kappa, alpha, beta, gamma, lambda)
  History = fit$Iterates
  rand_index_MFMSBM_seq = rep(NA, niterations)
  if(!is.null(Z)) rand_index_MFMSBM_seq = sapply(History, function(x) adj.rand.index(x$Z, Z))
  TDR_MFMSBM_seq =  lapply(tau, function(x) {
  ...
  FDR_MFMSBM_seq = lapply(tau, function(x) {
    infer = lapply(History, function(y) SBM_inference(y$q, x))
  ...
  return(list(RI_MFMSBM_seq = rand_index_MFMSBM_seq, TDR_MFMSBM_seq = TDR_MFMSBM_seq, FDR_MFMSBM_seq = FDR_MFMSBM_seq, TIME_MFMSBM_seq = elapse_mfmsbmseq))
}

mfmsbm_seq_main = function(){
  library(rslurm)
  indir <- Sys.getenv("INDIR", unset = file.path(getwd(), "Data"))
  outdir = Sys.getenv("OUTDIR", unset = file.path(getwd(), "Results/MFMSBM_SEQ"))
  ...
  sjob = slurm_apply(
    ...
    cpus_per_node = 1,
    global_objects = c(
      "loglike", "logmargs", "getDahl", "MFMSBM_seq_estimation", "SBM_inference"
    ),
    pkgs = c(
