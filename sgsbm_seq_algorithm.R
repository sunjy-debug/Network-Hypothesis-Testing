...
logmargs = function(Z, X, j, alpha, beta, rou, kappa, delta, xi)
{
  ...
  ## Output: m(A_j) in collapsed sampler for SG-SBM ##
...
## function for Collapsed sampler for SG-SBM (main algorithm)
SGSBM_seq_estimation = function(X, niterations, delta, xi, rou, kappa, alpha, beta, eta, zeta, m)
{
  ...
  ##        lambda \sim Stirling-Gamma(a, b, m) ##
  ...
  ##        rou, kappa = hyperparameters for the prior on elements in mu vector in Normal distribution ##
    ...
    { #determine whether ith component is a singleton (the only one node in the cluster)
      ...
      if (counts_Z[current.cluster.i] > 1){
        ...
        if (cluster.i > k)
        {
          ...
          sigma_star[k + 1, 1:(k + 1)] = sqrt(rinvgamma(k + 1, delta, scale = xi))
...
sgsbm_seq_algorithm = function(file_directory, output_directory, data_generation_method, random_seed, tau = c(0.005, 0.025, 0.05, 0.1, 0.15, 0.25)){
  ...
  cat("SGSBM(sequential) fitting begins. \n")
  start_sgsbmseq = Sys.time()
  niterations = 300
  delta = 3
  xi = 2
  rou = 0
  ...
  lambda = 1
  nrepetition = 5
  fit = SGSBM_seq_estimation(X, niterations, delta, xi, rou, kappa, alpha, beta, gamma, lambda, nrepetition)
  History = fit$Iterates
  rand_index_SGSBM_seq = rep(NA, niterations)
  if(!is.null(Z)) rand_index_SGSBM_seq = sapply(History, function(x) adj.rand.index(x$Z, Z))
  TDR_SGSBM_seq =  lapply(tau, function(x) {
  ...
  FDR_SGSBM_seq = lapply(tau, function(x) {
    infer = lapply(History, function(y) SBM_inference(y$q, x))
    FDR = sapply(infer, function(y) sum((1 - A) * y, na.rm = TRUE) / pmax(sum(y, na.rm = TRUE), 1))
    return(FDR)
  })
  end_sgsbmseq = Sys.time()
  elapse_sgsbmseq = end_sgsbmseq - start_sgsbmseq
  cat("SGSBM(sequential) fitting ends: \n")
  print(elapse_sgsbmseq)
  
  return(list(RI_SGSBM_seq = rand_index_SGSBM_seq, TDR_SGSBM_seq = TDR_SGSBM_seq, FDR_SGSBM_seq = FDR_SGSBM_seq, TIME_SGSBM_seq = elapse_sgsbmseq))
}

sgsbm_seq_main = function(){
  library(rslurm)
  indir <- Sys.getenv("INDIR", unset = file.path(getwd(), "Data"))
  outdir = Sys.getenv("OUTDIR", unset = file.path(getwd(), "Results/SGSBM_SEQ"))
  ...
  sjob = slurm_apply(
    ...
    cpus_per_node = 1,
    global_objects = c(
      "loglike", "logmargs", "getDahl", "SGSBM_seq_estimation", "SBM_inference"
    ),
    pkgs = c(
      "ConjugateDP", "invgamma", "fossil", "dplyr", "tidyr", "MASS"
    )
  )
