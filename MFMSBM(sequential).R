...
TDR_MFM_upper = pmin(TDR_MFM_mean + qnorm(.975) * TDR_sd, 1)
TDR_MFM_lower = pmax(TDR_MFM_mean - qnorm(.975) * TDR_sd, 0)
FDR_MFM_mean = apply(FDR_MFM, 2, mean)
FDR_MFM_sd = apply(FDR_MFM, 2, sd)
FDR_MFM_upper = pmin(FDR_MFM_mean + qnorm(.975) * FDR_sd, 1)
FDR_MFM_lower = pmax(FDR_MFM_mean - qnorm(.975) * FDR_sd, 0)
...
df_results = data.frame(
  ...
  mean = c(TDR_MFM_mean, FDR_mean),
  lower = c(TDR_MFM_lower, FDR_lower),
  upper = c(TDR_MFM_upper, FDR_upper)
)
